...
the following table. Spaces are allowed on either side of the “=”.
  ...
  **Known object**       **Purpose**
  ...
  obstruction\_extent    Even after a vehicle has passed an obstruction it needs additional span to show up as a blob past the obstruction in frame differencing operation.
  ...
  SENSITIVITY\_VALUE     Sensitivity value for the OpenCV absdiff funtion. It probably doesn't need to be tweaked, but it is a fine tuning parameter for the speed tracker. I set it early on and forgot it.
  BLUR\_SIZE             Used to smooth the intensity image output from absdiff() function. Again, I set it once and didn't need to tweak it.
